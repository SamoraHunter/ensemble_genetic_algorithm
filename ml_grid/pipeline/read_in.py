...
class read_sample:
    def __init__(
        ...
        if test_sample_n == 0 and column_sample_n == 0:
            self.raw_input_data = (
                None  # Set raw_input_data to None if both parameters are 0
            )
        elif test_sample_n == 0:
            self.raw_input_data = pd.read_csv(
                self.filename, usecols=lambda x: x in necessary_columns
            )  # Read only specified columns
        elif column_sample_n == 0:
            self.raw_input_data = pd.read_csv(
                self.filename, nrows=test_sample_n
            )  # Read only specified rows
        else:
            self.raw_input_data = pd.read_csv(
                self.filename,
                nrows=test_sample_n,
                usecols=lambda x: x in necessary_columns,
            )  # Read both rows and columns
