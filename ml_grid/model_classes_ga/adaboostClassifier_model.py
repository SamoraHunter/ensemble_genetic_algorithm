...
def AdaBoostClassifierModelGenerator(ml_grid_object, local_param_dict):
    """
    Generates an AdaBoostClassifier model using the grid search.

    Parameters
    ----------
    ml_grid_object : object
        The ml_grid object that contains all the data and parameters needed to run the model.
    local_param_dict : dict
        The dictionary of parameters used for this particular run of the model. This is generated by grid_param_space_ga.Grid(sample_n=n_iter).

    Returns
    -------
    tuple
        A tuple containing the following: (mccscore, model, feature_list, model_train_time, auc_score, y_pred)
    """
