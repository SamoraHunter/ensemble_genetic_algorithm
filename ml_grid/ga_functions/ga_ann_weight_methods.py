...
def get_y_pred_ann_torch_weighting(
    ...
    if ml_grid_object.verbose >= 11:
        logger.debug("get_y_pred_ann_torch_weighting")
    ...
    if valid:
        ...
        prediction_array = []
        range_iter = (
            tqdm(range(0, len(target_ensemble)))
            if getattr(ml_grid_object, "global_params", None)
            and ml_grid_object.global_params.progress_bars
            else range(0, len(target_ensemble))
        )
        for i in range_iter:
    ...
    if ml_grid_object.verbose >= 5:
        logger.info("Model training time: %s", model_train_time)
