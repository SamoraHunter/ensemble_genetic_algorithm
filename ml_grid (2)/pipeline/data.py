...
class pipe():
    ...
    def __init__(self, file_name, drop_term_list, local_param_dict, base_project_dir, param_space_index, additional_naming = None, test_sample_n=0): # kwargs**
        ...
        self.project_score_save_object = project_score_save_class(base_project_dir = base_project_dir)
        
        
        if(self.verbose>=1):
            print(f"Starting... {self.local_param_dict}")
    
        self.logging_paths_obj = log_folder(local_param_dict =local_param_dict,
        ...
        final_column_list = [self.X for self.X in self.pertubation_columns if (self.X not in self.drop_list)]
