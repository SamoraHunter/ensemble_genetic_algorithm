...
class TestHandleCorrelationMatrix(unittest.TestCase):
    ...
    def test_performance_scaling(self):
        ...
        result = handle_correlation_matrix(local_param_dict, [], df_test, chunk_size=50)
        ...
        conservative_estimate = (
            elapsed_time * (target_cols / n_cols_test) * (target_cols / n_cols_test / 2)
        )
        ...
        print(
            f"  Quadratic estimate: {estimated_time_15k:.2f} seconds ({estimated_time_15k/60:.2f} minutes)"
        )
        print(
            f"  Conservative estimate: {conservative_estimate:.2f} seconds ({conservative_estimate/60:.2f} minutes)"
        )
        print(f"{'='*70}")
        ...
        self.assertGreater(
            len(result), 0, "Should find at least some correlated columns"
        )
    ...
    def test_scaling_with_different_chunk_sizes(self):
        ...
        for chunk_size in [25, 50, 100]:
            ...
            result = handle_correlation_matrix(
                local_param_dict, [], df_test, chunk_size=chunk_size
            )
        ...
        for chunk_size, elapsed in times.items():
            self.assertLess(
                elapsed, 10, f"chunk_size={chunk_size} took too long: {elapsed:.2f}s"
            )
