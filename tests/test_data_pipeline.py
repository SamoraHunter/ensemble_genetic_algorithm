...
class TestDataPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data["correlated_blood_test_mean"] = (
            data["some_blood_test_mean"] * 0.9 + np.random.rand(100) * 0.1
        )
    ...
    def test_safety_net_prevents_no_features_error(self):
        ...
        pipeline = pipe(
            ...
            drop_term_list=[
                ...
                "sex_male",
                "categorical_feature",
            ],  # Drop everything by name EXCEPT 'age'
            local_param_dict=self.local_param_dict,
            base_project_dir=self.base_project_dir,
            param_space_index=0,
        )
        self.assertEqual(
            ...
            1,
            "Safety net should have retained exactly one feature.",
        )
        self.assertEqual(
            pipeline.X_train.columns[0],
            "age",
            "Safety net should retain 'age' as the only available numeric column.",
        )
    ...
    def test_string_column_raises_error_when_not_dropped(self):
        ...
        self.assertTrue(
            "non-numeric" in str(context.exception)
            or "string" in str(context.exception)
        )
    ...
    def test_feature_transformation_log(self):
        ...
        self.assertIn(
            "Initial Load", pipeline.feature_transformation_log["step"].values
        )
        self.assertIn(
            "Drop Correlated", pipeline.feature_transformation_log["step"].values
        )
        self.assertIn(
            "Drop Missing", pipeline.feature_transformation_log["step"].values
        )
        self.assertIn(
            "Drop Constants", pipeline.feature_transformation_log["step"].values
        )
